 ...
 * regardless of theme. Type and icons use `text-paper` / `bg-amber/15`
...
export function Differentiators() {
  ...
  return (
    ...
    >
      {/* Soft amber wash for warmth — works on both navy and cream */}
      <div
        aria-hidden
        className="absolute top-0 right-0 w-[28rem] h-[28rem] rounded-full bg-amber/15 blur-3xl pointer-events-none"
      />
      <div
        aria-hidden
        className="absolute bottom-0 left-0 w-[20rem] h-[20rem] rounded-full bg-amber/10 blur-3xl pointer-events-none"
      />
      ...
      <div className="relative max-w-6xl mx-auto px-5 md:px-10 py-20 md:py-28">
        ...
        <motion.div
          ...
          className="max-w-2xl"
        >
          <p className="text-xs uppercase tracking-[0.22em] text-amber mb-4">
            {eyebrow}
          </p>
          <h2 className="font-display text-4xl md:text-5xl lg:text-[3.5rem] leading-[1.04] tracking-tight text-paper">
            ...
            <em className="italic text-amber not-italic-tablet">gets wrong.</em>
          </h2>
        </motion.div>
        ...
        <ul className="mt-14 grid sm:grid-cols-2 gap-x-10 lg:gap-x-16 gap-y-10 lg:gap-y-12 max-w-5xl">
          {items.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-amber/15 border border-amber/40 text-amber shrink-0">
